...
let calculateTotal = function (...args: Array<number>): number {
  let total = 0;
  args.forEach((a) => {
    total += a;
  });
...
// interface와 type은 형식이 좀 다르다.
type Success = (url: string) => void;

// interface Success {
//   (url: string): void;
// }
type Fail = ({ message }: Error) => void;

type IMovePage = (url: string, success: Success, fail: Fail) => void;
// {}로 감싸면 : 를 안 감싸면 =>

const movePage: IMovePage = (url, success, fail) => {
...
const listData: Data<Array<string>> = {
  success: true,
